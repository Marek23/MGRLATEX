...
\label{ssec:crimMod}
Algorytm wypełniania fragmentami obrazu zbadano pod kątem ograniczenia źródła czerpania najlepszego dopasowania $q_{\hat{x}}$ do okna będącego otoczeniem o promieniu $r$ rozważanego piksela $x$. Wtedy dla $q$ zachodzi zależność $q \in \mathcal{Z}$, $\mathcal{Z} = r_x$, gdzie $r_x$ jest oznaczeniem analogicznym do $p_x$, z jedyną różnicą rozmiaru wyznaczanego fragmentu obszaru. Matematycznie zmiana odnosi się do dynamicznej zmiany pola $D^c$ jako zależność od rozważanego piksela $x$.
\subsection{Modyfikacja funkcji $C_t$ }
Podczas badań sprawdzono zmianę definicji oryginalnej dla algorytmu funkcji $C_t$ z zależności \eqref{confidenceTerm} na \eqref{pewnoscVFI}.
\subsection{Modyfikacja norm.}
Podczas badania algorytmu przetestowano różne rodzaje norm zastępując oryginalną \eqref{FROBENIUS2} normami: \eqref{NLWEIGHT}, \eqref{normNNF}, \eqref{nonLocalMeans}, \eqref{nonLocalMedians} czy \eqref{nonLocalpoisson}.
...
\label{LAPLASJAN}
\end{equation}
W celu uzyskania efektu dokładniejszego odwzorowania wektorów granic struktur wnikających wgłąb nieznanego obszaru, co za tym idzie poprawienia dokładności algorytmu, kolorowy obraz należy przekształcić do przestrzeni $CIELAB$ a następnie korzystając z warstwy jasności wyznaczyć wektor na podstawie wartości z przyjętego uprzednio rozmiaru skrawka obrazu. 
