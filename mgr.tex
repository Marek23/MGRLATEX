...
\glsxtrnewsymbol[description={promień tworzonego skrawka wokół piksela}]{pixradius}{$p_r$}
...
\begin{figure}[!h]
	\centering
	\includegraphics[scale=0.4]{rysunki/4_fig1}
...
\begin{figure}[!h]
	\centering
	\includegraphics[scale=0.7]{rysunki/4_fig3}
...
Podczas badania algorytmu bazującego na równaniu dyfuzji szukano rozwiązań opartych na różnych
sposobach dyskretyzacji. Dla \eqref{NavierAdv} jednym z nich może być metoda przedstawiona w \cite{van2005algorithms}, wykorzystująca asymetryczny schemat liczenia pierwszej pochodnej funkcji. Pierwsze pochodne lewostronne przyjmują postać:
\begin{equation}
...
\label{leftdfdy}
\end{equation}
Analogicznie pochodne prawostronne można zapisać jako:
\begin{equation}
...
\label{rightdfdy}
\end{equation}
Następnie powyższe zależności znajdują zastosowanie w wyznaczeniu wyrażeń \\
...
Algorytm wypełniania fragmentami obrazu zbadano pod kątem ograniczenia źródła czerpania najlepszego dopasowania $q_{\hat{x}}$ do okna będącego otoczeniem o promieniu $r$ rozważanego piksela $x$. Wtedy dla $q$ zachodzi zależność $q \in \mathcal{Z}$, $\mathcal{Z} = r_x$, gdzie $r_x$ jest oznaczeniem analogicznym do $p_x$, z jedyną różnicą rozmiaru wyznaczanego fragmentu obszaru. Matematycznie zmiana odnosi się do dynamicznej zmiany pola $D^c$ jako zależność od rozważanego piksela $x$. Modyfikację przedstawia \autoref{RysCrimMod}.
\begin{figure}[!h]
	\centering
...
Podczas badań wykorzystano definicję pewności co do piksela \eqref{pewnoscVFI} i podmieniono ją z oryginalnie wprowadzoną przez autorów \cite{criminisi2004region} zależnością $C_t$ \eqref{confidenceTerm}.
\subsection{Modyfikacja normy.}
Podczas badania algorytmu przetestowano różne rodzaje norm zastępując oryginalną wprowadzoną przez autorów \cite{criminisi2004region} \eqref{FROBENIUS2} normami: \eqref{NLWEIGHT}, \eqref{normNNF}, \eqref{nonLocalMeans}, \eqref{nonLocalMedians} oraz \eqref{nonLocalpoisson}.
\subsection{Modyfikacja funkcji $D_t$.}
Ze względu na nieregularny kształt większości masek wyznaczenie granicy $\partial \mathrm{\Omega }$ można wykonać wykorzystując operację erozji i dylacji na wspomnianej masce. Innym sposobem może być zastosowanie dyskretnego splotu maski z macierzą będącej laplasjanem dla siatki 8-spójnej: 
\begin{equation}
\left[ \begin{array}{ccc}
1 & 1 & 1 \\ 
1 & -8 & 1 \\ 
...
\label{LAPLASJAN}
\end{equation}
Aby polepszyć odwzorowanie wektorów: normalnego $n_x$ i $\nabla^\bot u_x$ (\autoref{4_fig3}) wnikających wgłąb nieznanego obszaru, co za tym idzie poprawić dokładność algorytmu, kolorowy obraz przekształcono do przestrzeni $CIELAB$, a następnie przy wykorzystaniu warstwy jasności wyznaczono wartość $D(x)$ biorąc pod uwagę różniczki wyznaczane dla wszystkich pikseli otaczających główny piksel (pikseli mieszczących się w skrawku utworzonym względem piksela $x$ o uprzednio definiowanym rozmiarze) przy jednoczesnym stopniowaniu ich procentowego wpływu na ostatecznie wyznaczaną wartość.
Przykładowo licząc $n_{p_{(i,j)}}$ dla macierzy o rozmiarze $3$:
\[
\begin{bmatrix}
n_{p_{(i,j)}}
\end{bmatrix}
...
\end{bmatrix}
\]
gdzie $c_1, c_2, c_3$ współczynniki stopniujące wpływ różniczek wyznaczane na podstawie rozkładu Gaussa.
...
\label{NLH1}
\end{align}
W powyższym równaniu eliminacja zmiennych $v$ oraz $b$ pozwala znacznie ograniczyć wykorzystywaną pamięć przez program oraz przyspieszyć działanie dzięki pominięciu obliczeń, wyrugowanych zmiennych. Zysk w czasie oraz różnicę w obliczonych obrazach przedstawia dział \ref{ssec:NLCTVModSec} 
\section{Połączenie syntezy obrazu z funkcją wagi.}
Kolejną modyfikacją algorytmu $NLCTV$ wykonaną przez autora jest zastąpienie problemu minimalizacji energii \eqref{ENLCTV}, metodą syntezy obrazu bazującej na metodzie syntezy tekstury opisanej w \autoref{chap:StructureTextureNavierStokes}. Kroki nowego algorytmu można przedstawić w następującym schemacie:
\begin{enumerate}
\item
Wyznaczenie funkcji wagi $w$ zgodnie z \eqref{DNLWEIGHT} 
\item
Powtarzanie aż do uzyskania zbioru $\Omega = \emptyset$
\begin{enumerate}[a)]
\item
Znalezienie dla każdego piksela $x \in \partial\Omega$ najlepszego dopasowania $\hat{x} \in D \cap \mathrm{N}_x$:
\begin{large}
\begin{align}
...
Przykładowe wyniki przedstawiono w tabeli:
\begin{longtable}[h!]{|c|c|}
    \hline
    Obraz & Dane \\ \hline
